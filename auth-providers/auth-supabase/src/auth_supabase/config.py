...
class SupabaseConfig(BaseSettings):
    ...
    auth_jwt_secret: str = Field(
        ...
        description=(
            "Supabase JWT secret for token validation, defaults to supabase "
            "local default value"
        ),
    ...
    supabase_public_key: str = Field(
        description=(
            "Supabase publishable key (new version of anon key), "
    ...
    supabase_secret_key: str = Field(
        description=(
            "Supabase secret key (new version of service role key), "
